...
<div class="wrapper">
  ...
  <div class="content-wrapper">
   ...
   <section class="col-lg-12">
      ...
      $no = mysqli_query($koneksi, " SELECT no_sk FROM t_nomor ORDER BY no_sk DESC LIMIT 1");
      $tampung = mysqli_fetch_assoc($no);
      //$_SESSION['sk'] = ((isset($_SESSION['sk'])) ? $_SESSION['sk'] : $tampung['no_sk'] );
      if (isset($_POST['pick'])) {
        //$_SESSION['sk']++; // menambah nilai satu pada session
        $nama = ($_POST['pengolah']);
        $nip = ($row['nip']);
        $email = ($row['email']);
        $no_hp = ($row['no_hp']);
        ...
        $cek = mysqli_query($koneksi,"SELECT * FROM t_nomor WHERE no_sk='$no_surat'");
        if(mysqli_num_rows($cek)==0){ //jika tidak ada yang sama maka kerjakan yang dibawah
        $insert = mysqli_query($koneksi, "INSERT INTO t_nomor (nama,nip,email,no_hp, tgl_ambil, perihal, kode_surat, tgl_surat, tujuan_kirim, no_sk) VALUES('$nama','$nip','$email','$no_hp',NOW(),'$perihal','$kode_surat','$tgl_surat','$tujuan_kirim','$no_surat')") or die (mysqli_error());//NOW digunakan untuk input tanggal dan waktu skarang
        if ($insert){
                 echo "<script>
                  swal({ 
    ...
    ?>
      <form action="" class="form-horizontal" method="post" enctype='multipart/form-data'>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-4">
            <input type="text" name="pengolah" class="form-control" value="" placeholder="PENGOLAH" required>
          </div>
        </div>
        <div class="form-group">
            ...
            <div class="modal-dialog" style="width:800px">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">Data Pengambil Nomor Surat SK</h4>
                    </div>
                    <div class="modal-body">
                      <table id="lookup" class="table table-bordered table-hover table-striped">
                            ...
                            <tbody>
                               <?php
                                    $query = mysqli_query($koneksi,'SELECT id, tgl_ambil, no_sk, nama, kode_surat, perihal, tgl_surat, tujuan_kirim FROM t_nomor WHERE no_sk>=1 ');
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo ' <a href="e_nosuratsk.php?id='.$data['id'].'"</a> Edit';?>                  
                                        </td>
                                    </tr>
                                    <?php
                                   }
