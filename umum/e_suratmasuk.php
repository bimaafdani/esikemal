     ...
     <section class="col-lg-12 connectedSortable">
          ...
          <?php
			...
			if(isset($_POST['save'])){
				...
				$update = mysqli_query($koneksi, "UPDATE surat_masuk SET no_agenda='$no_agenda', no_surat='$no_surat', asal='$asal', perihal= '$perihal', tgl_surat='$tgl_surat', tgl_diterima='$tgl_diterima', lampiran='$lampiran', status='$status', sifat='$sifat' WHERE idsurat='$idsurat'") or die(mysqli_error());
				if($update){
					echo "<script>alert('Surat Berhasil Dirubah'); window.location=history.go(-2)</script>";
				}else{
					echo '<div class="alert alert-danger">Data gagal disimpan, silahkan coba lagi.</div>';
				}
			}
			?>
			<form class="form-horizontal" enctype='multipart/form-data' method="post">	 <div class="box-body">
				<div class="form-group">
					<label class="col-sm-3 control-label">Nomor Agenda</label>
					<div class="col-sm-3">
						<input type="text" name="no_agenda" class="form-control" value= "<?php echo $row['no_agenda'];?>" required>
					</div>
				</div>
