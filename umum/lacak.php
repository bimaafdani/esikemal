      	...
      	<div class="box-body">   
         		 <div class="col-sm-12">
           			<div class="input-group">
           				...
           				<input type="text" name="inputan" class="form-control" placeholder="Masukkan Nomor Agenda">
            			<span class="input-group-btn">
			              <input  name="lacak" class="btn btn-success" data-toggle="modal" data-target="#myModal" value="Cari" type="submit">
			            </span>
            		</div>
            		<center><h3>Laporan Data Lacak Surat</h3></center>
          		</div>     
        </div>
    </form>  
  </div>
  <div class="box-body table-responsive no-padding">
        <table class="table table-striped">
                <tr>
                  ...
                  <th class="active"><center>Tanggal</th>
                  <th class="active"><center>Penerima</th>
                  <th class="active"><center>Tindakan</th>
                  <th class="active"><center>Status</th>
               </tr> 
              <?php
               ...
               $no = 0;
               	if($inputan==""){
               		echo "<script>alert('Masukkan Nomor Agenda Untuk Lacak Surat');</script>";
               	}elseif ($inputan!="") {
               		$sql = mysqli_query($koneksi, "SELECT * FROM surat_masuk WHERE no_agenda = '$inputan'");
                  if (mysqli_num_rows($sql) == 1 ){
                     ...
                     <tr>
                        ...
                        <td><center>'.$row['tgl_input'].'</td>
                        <td><center>Bagian Umum</td>
                        <td><center>...</td>
                        <td><center>'.$row['ket'].'</td> 
                    </tr>
                    ';
                   }
               }
               $sql = mysqli_query($koneksi, "SELECT * FROM t_kasubtu WHERE no_agenda = '$inputan'");
                  if (mysqli_num_rows($sql) == 1 ){
                  	while($row = mysqli_fetch_assoc($sql)){
                      echo '
                     <tr>
                        ...
                        <td><center>'.$row['tgl_input'].'</td>
                        <td><center>Kasubbag Tatausaha</td>
                        <td><center>...</td>
                        <td><center>Diterima</td>
               	...
               	$sql = mysqli_query($koneksi, "SELECT * FROM t_kepala WHERE no_agenda = '$inputan'");
                  if (mysqli_num_rows($sql) == 1 ){
                  	while($row = mysqli_fetch_assoc($sql)){
                      echo '
                     <tr>
                        ...
                        <td><center>'.$row['penerima'].'</td>
                        <td><center>...</td>
                        <td><center>Diproses</td>
