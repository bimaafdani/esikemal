...
<head>
  ...
  <!-- AdminLTE sppdins. Choose a sppdin from the css/sppdins
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Nomor Surat Keputusan (SK) KEPEGAWAIAN
      </h1>
      <ol class="breadcrumb">
   ...
   <section class="col-lg-12">
      ...
      $no = mysqli_query($koneksi, " SELECT no_sppd FROM t_nomor ORDER BY no_sppd DESC LIMIT 1");
      $tampung = mysqli_fetch_assoc($no);
      //$_SESSION['sppd'] = ((isset($_SESSION['sppd'])) ? $_SESSION['sppd'] : $tampung['no_sppd'] );
      if (isset($_POST['pick'])) {
        //$_SESSION['sppd']++; // menambah nilai satu pada session
        $nama = ($row['nama']);
        $nip = ($row['nip']);
        $email = ($row['email']);
        $no_hp = ($row['no_hp']);
        ...
        $no_surat = $tampung['no_sppd']+1;
        if ($tgl_surat != date ('Y-m-d')){
           echo "<script>
                          swal({ 
                          title:'GAGAL !!',
                          text:'Tanggal Surat Harus sama Dengan Tanggal Hari Ini, Periksa Kembali Ya :)',
                          type : 'error',
                           },
                            function(){
        ...
        $cek = mysqli_query($koneksi,"SELECT * FROM t_nomor WHERE no_sppd='$no_surat'");
        if(mysqli_num_rows($cek)==0){ //jika tidak ada yang sama maka kerjakan yang dibawah
        $insert = mysqli_query($koneksi, "INSERT INTO t_nomor (nama,nip,email,no_hp, tgl_ambil, perihal, kode_surat, tgl_surat, tujuan_kirim, no_sppd) VALUES('$nama','$nip','$email','$no_hp',NOW(),'$perihal','$kode_surat','$tgl_surat','$tujuan_kirim','$no_surat')") or die (mysqli_error());//NOW digunakan untuk input tanggal dan waktu sppdarang
        if ($insert){
                 echo "<script>
                  swal({ 
                ...
                function(){
                  window.location.href='sk_kepeg.php';
                }); </script>";
            ...
            <div class="modal-dialog" style="width:800px">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">Data Pengambil Nomor SK Kepegawaian</h4>
                    </div>
                    <div class="modal-body">
                      <table id="lookup" class="table table-bordered table-hover table-striped">
                            ...
                            <tbody>
                               <?php
                                    $level  =  $_SESSION['nama'];
                                    $query = mysqli_query($koneksi,"SELECT id, tgl_ambil, no_sppd, nama, kode_surat, perihal, tgl_surat, tujuan_kirim FROM t_nomor WHERE no_sppd>='1' AND nama='$level' ");
                                    while ($data = mysqli_fetch_assoc($query)) {
                                    ?>
                                    <tr>
                                        <td><?php echo date('d/m/Y',strtotime($data['tgl_ambil']));?></td>
                                        <td><?php echo $data['no_sppd']; ?></td>
                                        ...
                                        <td><?php echo ' <a href="e_skkepeg.php?id='.$data['id'].'"</a> Edit';?>                  
                                        </td>
                                    </tr>
                                    <?php
                                   }
